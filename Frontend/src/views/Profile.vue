				...
				<div class="container" style="height:50vh;background:lavender">
					<div v-if="contentNum === 3">
						<div>
							...
							<p id="myNickname">닉네임: <input  v-model="this.nickname" type="text"><button @click="checkNickname">중복확인</button></p>
							<p>이름: <input type="text" v-model="this.name"></p>
							<p>전화번호: <input type="text" v-model="this.phone" placeholder="핸드폰 번호를 입력해주세요"></p>
							<p>회원등급: {{ myProfile.roles[0].name }}</p>
							<button @click="saveMyData">저장</button>
						</div>
						<div>
							<h5>비밀번호 변경</h5>
							<input v-model="password" type="password" placeholder="변경할 비밀번호를 입력해주세요">
...
export default {
	...
	methods: {
		checkNickname() {
			...
			axios.get(`${this.$store.state.constants.SERVER}/user/checkusername/${this.myProfile.username}`)
				.then(response => {
